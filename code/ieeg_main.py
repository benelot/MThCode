...
for i in range(len(patient_id)):
    cc, ts, cctl = ieeg.fast_corr_numpy(patient_id[i], time_begin[i], duration, t_lag=.7)
    ieeg.fast_corr_numpy_plot(cc, ts, patient_id[i], time_begin[i])
    print('Mean abs. Corr. of' + patient_id[i] + ': ' + str(np.mean(np.abs(cc))))
    ccs.append(cc)
    tls.append(tls)
